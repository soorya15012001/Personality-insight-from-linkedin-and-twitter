...
def get_detail(s, pub_info, liu="", pub_id="", key=""):
    ...
    x = "".join(x)
    return [x, IB, company, post]


def index(key_name, text, model_l, model_J):
    disc_key_traits = {
        "D": "Strong Willed",
        "I": "Open Networker",
    ...
    if key_name != "":
        ...
        disc = ["D", "I", "S", "C"]
        mbti = ['ESFJ', 'ESFP', 'ISFJ', 'ISFP', 'INTJ', 'ENFP', 'ENFJ', 'ISTP', 'INFJ', 'ESTP', 'ESTJ', 'ENTJ', 'INTP', 'INFP', 'ISTJ', 'ENTP']

        z = {}
        zz = {}

        # print(label_J.predict(x))
        ...
        # dddd = dict(zz)
        # mbti = label_J.predict(clean(str(text)))
        ...
        if (com < 0.5):
            ...
            tx["disc_OP"] = str(z[0][0] + str(z[1][0]))
            tx["mbti_OP"] = str(zz[0][0])
            tx["engagementIntelligence"] = eng
            tx["summary"] = summary
            tx["do"] = do
            tx["dont"] = dont
            tx["meetingIntelligence"] = meet
            tx["closureIntelligence"] = close
            tx["final_accuracy"] = float(z[0][1])
            ...
            tx["disc_OP"] = str(z[0][0] + str(z[1][0]).lower())
            tx["mbti_OP"] = str(zz[0][0])
            tx["engagementIntelligence"] = eng
            tx["summary"] = summary
            tx["do"] = do
...
data = pd.read_csv("input_2.csv", encoding='unicode_escape')
key = data["key"]
text = data["text"]


# disc = ['lstm.h5', 'lstm_unbiased.h5', 'lstm_FL.h5',
#         'cnn_CE_B.h5', 'cnn_CE_UB.h5', 'cnn_FL_B.h5', 'cnn_FL_UB.h5', 'cnn_CE_B_SM.h5',
#         'lenet_DISC.h5', 'CE_sigmoid_NB.h5', 'CE_softmax_NB.h5', 'FL_sigmoid_NB.h5', 'FL_softmax_NB.h5', 'lenet_NB.h5', 'lstm_NB.h5']
#
# mbti = ['lstm_MBTI.h5', 'lstm_MBTI_unbiased.h5', 'lstm_MBTI_FL.h5',
#         'cnn_MBTI_CE_B.h5', 'cnn_MBTI_CE_UB.h5', 'cnn_MBTI_FL_B.h5', 'cnn_MBTI_FL_UB.h5', 'cnn_MBTI_CE_B_SM.h5',
#         'lenet_MBTI.h5', 'CE_sigmoid_NB_mbti.h5', 'CE_softmax_NB_mbti.h5', 'FL_sigmoid_NB_mbti.h5', 'FL_softmax_NB_mbti.h5', 'lenet_NB_mbti.h5', 'lstm_NB_mbti.h5']

disc = ['cnn_CE_B_SM.h5', 'cnn_FL_UB.h5']

mbti = ['cnn_MBTI_CE_B_SM.h5', 'cnn_MBTI_FL_UB.h5']

cleaned = []
for i in list(text):
    cleaned.append(clean_demo(i))
pd.DataFrame(cleaned).to_csv("clean_text.csv")

for d, m in zip(disc, mbti):
    fin = []

    model_l = load_model("models/"+d)
    label_J = load_model("models/"+m)
    print(d)
    for i, l in tqdm(zip(key, text)):
        if i == "####":
            break
        else:
            tx = index(key_name=i, text=l, model_l=model_l, model_J=label_J)
            fin.append(tx)
    print(fin)
    pd.DataFrame(fin).to_csv(d+"verify_text.csv")
