...
data = pd.read_csv('input_linkedin.csv')
name = data["key"]
li_id = data["text"]

for i in li_id:
    op = aa(liu="SudipDutta", req_by="Sudip Dutta", pub_id=i, key_name="", key_company="", key_role="", twitter_id="",
    ...
    if len(op) >5:
        disc = op["disc_info"]["disc"]
        mbti = op["mbti_info"]["mbti"]
        engint = op["EngIntel"]
        summ = op["summary"]
        do = op["do"]
        dont = op["dont"]
        meet = op["meet_close"]["meetingIntelligence"]
        close = op["meet_close"]["closureIntelligence"]
        com_ib = op["ice_breakers"]["common_IB"]
        uncom_ib = op["ice_breakers"]["uncommon_IB"]
        snap_date = op["snapshot_date_time"]
        req_by = op["snapshot_by"]
        ret_time = op["snaphot_retrieval_time"]


        name = op["contact_info_linkedin"]["first_name"] + " " + op["contact_info_linkedin"]["last_name"]
        city = op["contact_info_linkedin"]["location"]["city"]
        country = op["contact_info_linkedin"]["location"]["country"]
        curr_desig = op["contact_info_linkedin"]["current_company"]["detail"]["title"]
        curr_comp = op["contact_info_linkedin"]["current_company"]["name"]
        curr_comp_since = op["contact_info_linkedin"]["current_company"]["detail"]["timePeriod"]
        schools = op["contact_info_linkedin"]["schools"]
        companies = op["contact_info_linkedin"]["companies"]
        lin_post_url = op["contact_info_linkedin"]["latest_linkedin_post"]["url"]
        lin_post_date = op["contact_info_linkedin"]["latest_linkedin_post"]["time_period"]
        lin_bio = op["contact_info_linkedin"]["about"]
        email = op["contact_info_linkedin"]["email"]
        ph_no = op["contact_info_linkedin"]["phone_number"]
        profile_acc = op["contact_info_linkedin"]["profile_confidence"]


        try:
            tw_url = "https://twitter.com/"+ op["contact_info_twitter"]["contact_info"]["handle"]
            tw_latest = op["contact_info_twitter"]["latest_tweet"]["tweet_text"]
            tw_latest_url = op["contact_info_twitter"]["latest_tweet"]["tweet_url"]
            tw_latest_date = op["contact_info_twitter"]["latest_tweet"]["tweet_date"]
            tw_handle = op["contact_info_twitter"]["contact_info"]["handle"]
            img = op["contact_info_twitter"]["contact_info"]["img"]
        ...
        # fin_op = {"name": name, "disc": disc, "mbti": mbti, "image_url": img, "city": city, "country": country, "current_designation": curr_desig,
        #           "current_company": curr_comp, "current_company_timeperiod": curr_comp_since, "engagement_intelligence": engint,
        #           "summary": summ, "do": do, "dont": dont, "meet": meet, "close": close, "common_icebreaker": com_ib,
        #           "uncommon_icebreaker": uncom_ib, "schools": schools, "companies": companies, "linkedin_post_url": lin_post_url,
        #           "linkedin_post_date": lin_post_date, "linkedin_bio": lin_bio, "twitter_url": tw_url, "latest_tweet": tw_latest,
        #           "latest_tweet_url": tw_latest_url, "latest_tweet_date": tw_latest_date, "twitter_handle": tw_handle,
        #           "email": email, "ph_no": ph_no, "profile_acc": profile_acc, "snap_date_time": snap_date, "requested_by": req_by,
        #           "retrieval_time": ret_time, "text": op["AnalyzedText"]}

        fin_op = {"name": name, "text": op["AnalyzedText"], "single": disc + "-" + mbti}

    else:
        fin_op = {"name": name, "text": "insufficient text", "single": "insufficient text"}

    print(fin_op)
    with open("verify_linkedin.txt", "a", encoding="utf-8") as myfile:
        myfile.write(str(fin_op)+",\n\n")
